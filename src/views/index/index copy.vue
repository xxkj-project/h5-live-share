...
import { videoPullUrl, roomIds, appDownloadUrl } from '@/const/global'
...
export default {
  ...
  methods: {
    ...
    initSocket() {
      this.ws = socket.initSocket((type, data) => {
        switch (type) {
          case 'open':
            this.onOpen()
            break
    ...
    onClose(e) {
      ...
      if (this.socketCode === 1) this.handleReconnect()
    },
    onError(e) {
      console.log('-连接出错了-', e)
      if (this.socketCode === 1) this.handleReconnect()
